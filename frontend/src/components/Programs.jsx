      ...
      <div className="max-w-7xl mx-auto px-6">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
          {programs.map((p) => (
            <div
              key={p.name}
              className="bg-white p-10 rounded-3xl shadow-lg shadow-gray-200/50 hover:-translate-y-2 transition-all"
            >
              <div
              ...
              <Link
                to="inquiry"
                smooth={true}
                className="inline-block px-6 py-3 bg-[#4D9699] hover:bg-opacity-90 text-white font-bold rounded-full cursor-pointer transition-all"
              >
                Select Program
              </Link>
            </div>
